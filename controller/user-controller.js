...
const bcryptjs = require('bcryptjs');
const  User  = require('../models/user-model.js'); // 引入你的 Sequelize 模型
...
const signup = async (req, res, next) => {
  ...
  try {
    ...
    if (existingUser) {
      logger.error('Email has already been used');
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    if (!loggedInUser) {
      logger.error('Invalid credentials, user does not exist.');
      throw new HttpError('Invalid credentials, user does not exist.', 401);
    }

    const checkPassword = bcryptjs.compareSync(password, loggedInUser.password);
    if (!checkPassword) {
      logger.error('Invalid credentials, please check your email and password.');
      throw new HttpError('Invalid credentials, please check your email and password.', 401);
    }
...
const getUserProfile = async (req, res, next) => {
  ...
  try {
    const userProfile = await User.findByPk(userId, {
      attributes: ['id', 'name', 'email', 'place', 'age'],
    });
